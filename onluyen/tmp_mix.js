
  // =========================
  // PY10 PRETEST BRIDGE (để khóa/mở phần CODE ở trang chính)
  // =========================
  const PY10_SESSION_KEY = "py10:session";
  function py10GetSession(){
    try{ return JSON.parse(localStorage.getItem(PY10_SESSION_KEY) || "null"); }catch(_){ return null; }
  }
  const __py10Sess = py10GetSession();
  const __py10Scope = (__py10Sess && __py10Sess.id) ? String(__py10Sess.id).trim() : "guest";

  const PRETEST_ID = "mix";
  const PRETEST_PASS_SCORE = 1.0; // điểm/10
  const PRETEST_PASS_KEY = `py10:pretest:${__py10Scope}:${PRETEST_ID}:passed`;

  
  const PRETEST_RESULT_KEY = `py10:pretest:${__py10Scope}:${PRETEST_ID}:result`;

  // Lưu kết quả lần nộp gần nhất (kể cả chưa đạt) để giáo viên xem ở trang Kết quả
  function savePretestResult(payload, opts){
    try{
      const o = opts || {};
      const countAttempt = (o.countAttempt === undefined) ? true : !!o.countAttempt;
      const prev = (()=>{ try{ return JSON.parse(localStorage.getItem(PRETEST_RESULT_KEY) || "null") || {}; }catch(e){ return {}; } })();
      const attemptsPrev = Number(prev.attempts||0) || 0;
      const attempts = attemptsPrev + (countAttempt ? 1 : 0);
      const ts = (!countAttempt && prev && prev.ts) ? prev.ts : Date.now();
      const data = Object.assign({}, prev, payload || {}, { attempts, ts });
      localStorage.setItem(PRETEST_RESULT_KEY, JSON.stringify(data));
    }catch(e){}
  }

  // HS bấm "Làm code" => đóng modal ở trang chính (nếu đang mở trong iframe)
  function goToCode(){
    try{
      if(window.parent && window.parent !== window){
        window.parent.postMessage({ type:"py10_pretest_go_code", pretestId: PRETEST_ID, scope: __py10Scope }, "*");
        return;
      }
    }catch(e){}
    // Fallback: quay về trang chính
    try{ window.location.href = "../index.html"; }catch(e){}
  }
function markPretestPassed(score10, opts){
    try{
      const o = opts || {};
      const preserveTs = !!o.preserveTs;
      let prev = null;
      try{ prev = JSON.parse(localStorage.getItem(PRETEST_PASS_KEY) || "null"); }catch(e){ prev = null; }
      const ts = (preserveTs && prev && prev.ts) ? prev.ts : Date.now();
      const payload = { passed:true, score: Number(score10||0), ts };
      localStorage.setItem(PRETEST_PASS_KEY, JSON.stringify(payload));
    }catch(e){}
    // Thông báo cho trang cha (nếu đang mở trong iframe)
    try{
      if(window.parent && window.parent !== window){
        window.parent.postMessage({ type:"py10_pretest_pass", pretestId: PRETEST_ID, scope: __py10Scope, score: Number(score10||0) }, "*");
      }
    }catch(e){}
  }


  // =========================
  // DỮ LIỆU CÂU HỎI (từ Excel)
  // =========================
  const MCQ_BANK = [
  {
    "id": 101,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 1,
    "text": "Trong ngôn ngữ Python, tên nào sau đây là từ khoá?",
    "options": {
      "A": "program, sqr",
      "B": "uses, var",
      "C": "include, const",
      "D": "if, else."
    },
    "correct": "D"
  },
  {
    "id": 102,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 2,
    "text": "Đâu là quy tắc đúng khi đặt tên biến cho Python?",
    "options": {
      "A": "Tên biến có thể bắt đầu bằng dấu gạch dưới “_”",
      "B": "Có thể sử dụng keyword làm tên biến",
      "C": "Tên biến có thể bắt đầu bằng một chữ số",
      "D": "Tên biến có thể có các kí hiệu @, #, %, &,…"
    },
    "correct": "A"
  },
  {
    "id": 103,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 3,
    "text": "Từ khoá của một ngôn ngữ lâp trình là:",
    "options": {
      "A": "là những từ dành riêng",
      "B": "cho một mục đích sử dụng nhất định",
      "C": "có thể đặt tên cho biến",
      "D": "Cả A và B"
    },
    "correct": "D"
  },
  {
    "id": 104,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 4,
    "text": "Biến trong Python dùng để làm gì?",
    "options": {
      "A": "Lưu trữ dữ liệu",
      "B": "Hiển thị dữ liệu",
      "C": "So sánh dữ liệu",
      "D": "Xóa dữ liệu"
    },
    "correct": "A"
  },
  {
    "id": 105,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 5,
    "text": "Trong ngôn ngữ Python, tên nào sau đây đặt sai theo quy tắc?",
    "options": {
      "A": "11tinhoc",
      "B": "tinhoc11",
      "C": "tin_hoc",
      "D": "_11"
    },
    "correct": "A"
  },
  {
    "id": 106,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 6,
    "text": "Lệnh gán giá trị cho biến b nguyên nào sau đây là đúng?",
    "options": {
      "A": "b = 10",
      "B": "B = 10",
      "C": "b == 10",
      "D": "b = ‘10’"
    },
    "correct": "A"
  },
  {
    "id": 107,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 7,
    "text": "Trường hợp nào sau đây không phải lệnh gán trong python?",
    "options": {
      "A": "cd = 50",
      "B": "a = a * 2",
      "C": "a = 10",
      "D": "a + b = 100"
    },
    "correct": "D"
  },
  {
    "id": 108,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 8,
    "text": "Tên biến nào sau đây là đúng trong Python?",
    "options": {
      "A": "–tich",
      "B": "tong@",
      "C": "1_dem",
      "D": "ab_c1"
    },
    "correct": "D"
  },
  {
    "id": 109,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 9,
    "text": "Tìm lỗi sai trong khai báo biến nguyên Max sau đây? Max = 2021:",
    "options": {
      "A": "Dư dấu (=)",
      "B": "Tên biến trùng với từ khoá",
      "C": "Dư dấu (:)",
      "D": "Câu lệnh đúng"
    },
    "correct": "C"
  },
  {
    "id": 110,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 10,
    "text": "Để gán giá trị A cho một biến x thực hiện như thế nào?",
    "options": {
      "A": "x := A",
      "B": "A = x",
      "C": "x= A",
      "D": "A := x"
    },
    "correct": "C"
  },
  {
    "id": 111,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 11,
    "text": "Để tính diện tích đường tròn bán kính R, với pi = 3,14, biểu thức nào sau đây trong Python là đúng?",
    "options": {
      "A": "S:=R*R*pi",
      "B": "S=R*R*pi",
      "C": "S:=2(R)*pi",
      "D": "S:=R2*pi"
    },
    "correct": "B"
  },
  {
    "id": 112,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 12,
    "text": "Để viết 3 mũ 4 trong Python chọn",
    "options": {
      "A": "3**4",
      "B": "3//4",
      "C": "3*3+3*3",
      "D": "3%4"
    },
    "correct": "A"
  },
  {
    "id": 113,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 13,
    "text": "Chuyển biểu thức sau sang python",
    "options": {
      "A": "2*x+1/x+2",
      "B": "(2*x+1)/(x+2)",
      "C": "(2*x+1)(x+2)",
      "D": "(2*x+1) :(x+2)"
    },
    "correct": "B"
  },
  {
    "id": 114,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 14,
    "text": "Kết quả khi thực hiện câu lệnh sau là gì?\n\n>>> 10 - 5 ** 2 + 8//3 + 2",
    "options": {
      "A": "-11",
      "B": "11",
      "C": "7",
      "D": "Câu lệnh bị lỗi"
    },
    "correct": "A"
  },
  {
    "id": 115,
    "section": "Bài 1: Biến và lệnh gán",
    "no": 15,
    "text": "Biểu thức a/(a+1)*(x-1) khi chuyển sang dạng toán học (viết rõ ngoặc) có dạng:",
    "options": {
      "A": "a/(a+1)*(x-1)",
      "B": "(a/(a+1))*(x-1)",
      "C": "a/(a+1*(x-1))",
      "D": "a/((a+1)*(x-1))"
    },
    "correct": "B"
  },
  {
    "id": 201,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 1,
    "text": "Đưa dữ liệu ra màn hình dùng thủ tục nào?",
    "options": {
      "A": "print()",
      "B": "input()",
      "C": "type()",
      "D": "abs()"
    },
    "correct": "A"
  },
  {
    "id": 202,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 2,
    "text": "Câu lệnh nào dùng để đưa dữ liệu từ bàn phím vào?",
    "options": {
      "A": "print()",
      "B": "input()",
      "C": "nhap()",
      "D": "enter()"
    },
    "correct": "B"
  },
  {
    "id": 203,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 3,
    "text": "Phát biểu nào sau đây là sai?",
    "options": {
      "A": "Lệnh input() có chức năng nhập dữ liệu từ thiết bị chuẩn vào bàn phím",
      "B": "Bàn phím là thiết bị chuẩn duy nhất",
      "C": "Nội dung nhập có thể là số",
      "D": "Kết quả của lệnh input() là một xâu kí tự"
    },
    "correct": "B"
  },
  {
    "id": 204,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 4,
    "text": "1) Cú pháp lệnh input() : <biến> :=input(<Dòng thông báo>)\n2) Lệnh print() có chức năng đưa dữ liệu ra thiết bị chuẩn, thường là bàn phím\n3) Lệnh input() có chức năng nhập dữ liệu thường từ bàn phím\n4) Thông tin cần đưa ra có thể gồm nhiều kiểu dữ liệu khác nhau, cho phép cả biểu thức tính toán \n\nSố phát biểu đúng là",
    "options": {
      "A": "1",
      "B": "2",
      "C": "3",
      "D": "4"
    },
    "correct": "B"
  },
  {
    "id": 205,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 5,
    "text": "Đâu không phải là kiểu dữ liệu cơ bản trong python ?",
    "options": {
      "A": "int",
      "B": "float",
      "C": "list",
      "D": "string"
    },
    "correct": "C"
  },
  {
    "id": 206,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 6,
    "text": "Kết quả của dòng lệnh sau là gì?\n\n>>> x = 6.7\n>>> type(x)",
    "options": {
      "A": "int",
      "B": "float",
      "C": "string",
      "D": "double"
    },
    "correct": "B"
  },
  {
    "id": 207,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 7,
    "text": "Kết quả của dòng lệnh sau là gì?\n\n>>> x, y, z = 10, \"10\", 10\n>>> type(z)",
    "options": {
      "A": "int",
      "B": "float",
      "C": "double",
      "D": "str"
    },
    "correct": "A"
  },
  {
    "id": 208,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 8,
    "text": "Lệnh nào dùng để nhận biết kiểu dữ liệu của biến trong python?",
    "options": {
      "A": "type()",
      "B": "int()",
      "C": "size()",
      "D": "abs()"
    },
    "correct": "A"
  },
  {
    "id": 209,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 9,
    "text": "Xác định kiểu của biểu thức sau? “34 + 28 – 45 ”",
    "options": {
      "A": "int",
      "B": "float",
      "C": "bool",
      "D": "string"
    },
    "correct": "D"
  },
  {
    "id": 210,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 10,
    "text": "Xác định kiểu và giá trị của biểu thức sau 4 + 5*6-34 >5*8-2",
    "options": {
      "A": "bool, True",
      "B": "bool, true",
      "C": "bool, False",
      "D": "không xác định, false"
    },
    "correct": "C"
  },
  {
    "id": 211,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 11,
    "text": "Câu lệnh sau bị lỗi không?\n\n>>> int(10.5)",
    "options": {
      "A": "Không có lỗi",
      "B": "Câu lệnh có lỗi",
      "C": "Không xác định",
      "D": "Cả 3 phương trên đều sai"
    },
    "correct": "A"
  },
  {
    "id": 212,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 12,
    "text": "Kết quả của câu lệnh sau là gì?\n\n>>> str(3 + 4//3)",
    "options": {
      "A": "\"3+4//3\"",
      "B": "\"4\"",
      "C": "4",
      "D": "'4'"
    },
    "correct": "D"
  },
  {
    "id": 213,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 13,
    "text": "Để nhập giá trị số thực cho biến x, bạn An viết câu lệnh như sau:\n\nx = input(\"Nhập số thực x: \")\n\nCâu lệnh trên đúng theo yêu cầu đặt ra chưa?",
    "options": {
      "A": "Chương trình chạy đúng",
      "B": "Chương trình báo lỗi không chạy",
      "C": "Không xác định được lỗi",
      "D": "Chương trình vẫn chạy nhưng không đúng yêu cầu đặt ra "
    },
    "correct": "D"
  },
  {
    "id": 214,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 14,
    "text": "Viết chương trình nhập vào 4 số và tính tổng của chúng. Các dòng lệnh số mấy bị sai?\n\na = int(input(\"Nhập số a\"))\nb = float(input(\"Nhập số b\"))\nc = int(input(\"Nhập số c\"))\nd = input(\"Nhập số d\")\nprint(\"Tổng là: \", a + b + c + d)",
    "options": {
      "A": "Dòng 1, 2",
      "B": "Dòng 2, 4",
      "C": "Dòng 3, 5",
      "D": "Dòng 4"
    },
    "correct": "D"
  },
  {
    "id": 215,
    "section": "Bài 2: Các lệnh vào ra đơn giản",
    "no": 15,
    "text": "Câu lênh nào sau đây không báo lỗi? 1) float(4) 2) int(“1+3”) 3) int(“3”) 4) float(“1+2+3”)",
    "options": {
      "A": "1",
      "B": "2",
      "C": "3",
      "D": "4"
    },
    "correct": "A"
  },
  {
    "id": 301,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 1,
    "text": "Phát biểu nào sau đây là sai khi nói về biểu thức lôgic?",
    "options": {
      "A": "Biểu thức lôgic là biểu thức chỉ nhận giá trị True hoặc False",
      "B": "Giá trị của biểu thức lôgic thuộc kiểu bool",
      "C": "Ngoài hai giá trị True, False biểu thức lôgic nhận giá trị undefined",
      "D": "Biểu thức \"2 * 3 // 5 == 1\" mang giá trị True"
    },
    "correct": "C"
  },
  {
    "id": 302,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 2,
    "text": "Biểu diễn nào sau đây là sai trong Python?",
    "options": {
      "A": "b*b>a*c, a*(1-a)+(a-b)>=0; 1/x-x<0",
      "B": "(a-b)>c-d, 1//x-y>=2*x,b*b>a*c",
      "C": "(a-b)>c-d, (a-b)<>b-a,12*a>5a",
      "D": "(a-b)**0.5>x,1/x-y>=2*x,15*a>5"
    },
    "correct": "C"
  },
  {
    "id": 303,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 3,
    "text": "Kết quả của đoạn chương trình sau: x=2021 print((x%4==0 and x%100!=0) or x%400==0)",
    "options": {
      "A": "55",
      "B": "True",
      "C": "5",
      "D": "False"
    },
    "correct": "D"
  },
  {
    "id": 304,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 4,
    "text": "Cho x = True, y = False. Cho biết giá trị của biểu thức sau: not((x or y ) and x )",
    "options": {
      "A": "True",
      "B": "False",
      "C": "x",
      "D": "1"
    },
    "correct": "B"
  },
  {
    "id": 305,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 5,
    "text": "Giá trị của ai biểu thức sau là True hay False? 50%3==1 34//5==6",
    "options": {
      "A": "True, True",
      "B": "False, False",
      "C": "True, False",
      "D": "False, True"
    },
    "correct": "D"
  },
  {
    "id": 306,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 6,
    "text": "Phát biểu nào sau đây bị sai?",
    "options": {
      "A": "Sau <điều kiện> cần có dấu \":\"",
      "B": "Khối lệnh tiếp theo không bắt buộc lùi vào 1 tab và thẳng hàng",
      "C": "Khi thực hiện lệnh, Python sẽ kiểm tra <điều kiện> nếu đúng thì thực hiện <khối lệnh>, ngược lại thì bỏ qua chuyến sang lệnh tiếp theo",
      "D": "Có phát biểu chưa đúng trong ba phát biểu trên"
    },
    "correct": "B"
  },
  {
    "id": 307,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 7,
    "text": "Tìm giá trị m và n thoả mãn (m+4) % 5 == 0 và n*2//3=5",
    "options": {
      "A": "m = 1, n = 8",
      "B": "m = 2, n = 9",
      "C": "m = 3, n = 10,",
      "D": "m = 0, n = 7"
    },
    "correct": "A"
  },
  {
    "id": 308,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 8,
    "text": "Biểu thức lôgic đúng thể hiện số a nằm ngoài [3,8] là:",
    "options": {
      "A": "a < 3 and a > 8",
      "B": "3 <= a <= 8",
      "C": "a < 3 or a > 8",
      "D": "a <= 3 and a >= 8"
    },
    "correct": "C"
  },
  {
    "id": 309,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 9,
    "text": "Biểu thức lôgic nào sau đây thể hiện số a thuộc nửa khoảng (5, 7]",
    "options": {
      "A": "(5 < a) and (a<= 7)",
      "B": "5<= a <=7",
      "C": "5 < a < 7",
      "D": "5 <= a < 7"
    },
    "correct": "A"
  },
  {
    "id": 310,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 10,
    "text": "Kết quả của chương trình sau là gì?\n\nx = 5\ny = 6\nif x > y:\n    print('Max:', x)\nelse:\n    print('Max:', y)",
    "options": {
      "A": "Max:5",
      "B": "Max:6",
      "C": "Max: 5",
      "D": "Max: 6"
    },
    "correct": "D"
  },
  {
    "id": 311,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 11,
    "text": "Kết quả của chương trình sau là gì?\n\nx = 8\ny = 9\nif x > y:\n    print('x lớn hơn y')\nelif x == y:\n    print('x bằng y')\nelse:\n    print('x nhỏ hơn y')",
    "options": {
      "A": "x lớn hơn y",
      "B": "x bằng y",
      "C": "x nhỏ hơn y",
      "D": "Chương trình bị lỗi"
    },
    "correct": "C"
  },
  {
    "id": 312,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 12,
    "text": "Kết quả của chương trình sau là gì?\n\nnum = 3\nif num > 0:\n    print(num)\n    print(\"Thông điệp này luôn được in.\")\nnum = -1\nif num > 0:\n    print(num)\nprint(\"Thông điệp này cũng luôn được in.\")",
    "options": {
      "A": "3\nThông điệp này luôn được in.\nThông điệp này cũng luôn được in.",
      "B": "3\nThông điệp này luôn được in.\n-1\nThông điệp này luôn được in.",
      "C": "3\n-1\nThông điệp này luôn được in.",
      "D": "Thông điệp này luôn được in.\n-1\nThông điệp này luôn được in."
    },
    "correct": "A"
  },
  {
    "id": 313,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 13,
    "text": "Năm nhuận là năm chia hết cho 400 hoặc n chia hết cho 4 đồng thời không chia hết cho 100. Biểu thức biểu diễn kiểm tra năm nhuận là:",
    "options": {
      "A": "n % 4 == 100 or ( n % 4 == 0 and n % 100 == 0)",
      "B": "n % 4 == 100 or n % 4 == 0 and n % 100 != 0",
      "C": "n % 4 != 100 or ( n % 4 == 0 and n % 100 != 0)",
      "D": "n % 4 == 100 or ( n % 4 == 0 and n % 100 != 0)"
    },
    "correct": "D"
  },
  {
    "id": 314,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 14,
    "text": "Kết quả của lệnh print(round(4.5679,2)) là:",
    "options": {
      "A": "4.5",
      "B": "4.6",
      "C": "4.56",
      "D": "4.57"
    },
    "correct": "D"
  },
  {
    "id": 315,
    "section": "Bài 3: Câu lệnh rẽ nhánh if",
    "no": 15,
    "text": "Kết quả của biểu thức round(-1.232154, 4) là:",
    "options": {
      "A": "-1.2321",
      "B": "-1.2322",
      "C": "-1.23",
      "D": "-1,232"
    },
    "correct": "B"
  },
  {
    "id": 401,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 1,
    "text": "Số công việc cần phải lặp với số lần xác định?\n1) Đổ 10 xô nước vào thùng\n2) Đếm số chia hết cho 5 trong đoạn từ 10 tới 100.\n3) In tên lần lượt từng học sinh của một lớp có 30 em.\n4) Chạy 5 vòng sân bóng.\n5) Tính tổng các số có 2 chữ số.",
    "options": {
      "A": "3",
      "B": "4",
      "C": "5",
      "D": "2"
    },
    "correct": "C"
  },
  {
    "id": 402,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 2,
    "text": "Biến chạy trong vòng lặp for i in range(<giá trị cuối>) tăng lên mấy đơn vị sau mỗi lần lặp?",
    "options": {
      "A": "1",
      "B": "2",
      "C": "0",
      "D": "Tất cả đều sai"
    },
    "correct": "A"
  },
  {
    "id": 403,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 3,
    "text": "Cho các câu lệnh sau, hãy chỉ ra câu lệnh đúng:",
    "options": {
      "A": "x = 0\nfor i in range(10):\n    x = x + 1",
      "B": "x = 0\nfor i in range(10):\n    x := x + 1",
      "C": "x = 0\nfor i in range(10)\n    x = x + 1",
      "D": "x := 0\nfor i in range(10):\n    x = x + 1"
    },
    "correct": "A"
  },
  {
    "id": 404,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 4,
    "text": "Trong Python, câu lệnh nào dưới đây được viết đúng?",
    "options": {
      "A": "for i in range(10): prin(\"A\")",
      "B": "for i in range(10): print(\"A\")",
      "C": "for i in range(10): print(A)",
      "D": "for i in range(10) print(\"A\")"
    },
    "correct": "B"
  },
  {
    "id": 405,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 5,
    "text": "Trong câu lệnh lặp:\n\nj = 0\nfor j in range(10):\n    j = j + 2\nprint(j)\n\nKhi kết thúc câu lệnh trên, câu lệnh print(j) được thực hiện bao nhiêu lần?",
    "options": {
      "A": "10 lần",
      "B": "1 lần",
      "C": "5 lần",
      "D": "Không thực hiện"
    },
    "correct": "B"
  },
  {
    "id": 406,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 6,
    "text": "Trong câu lệnh lặp:\n\nj = 0\nfor j in range(10):\n    print(\"A\")\n\nKhi kết thúc câu lệnh trên lệnh có bao nhiêu chữ \"A\" xuất hiện?",
    "options": {
      "A": "10 lần",
      "B": "1 lần",
      "C": "5 lần",
      "D": "Không thực hiện"
    },
    "correct": "A"
  },
  {
    "id": 407,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 7,
    "text": "Cho đoạn chương trình:\n\nj = 0\nfor i in range(5):\n    j = j + i\nprint(j)\n\nSau khi thực hiện chương trình giá trị của biến j bằng bao nhiêu?",
    "options": {
      "A": "10",
      "B": "12",
      "C": "15",
      "D": "14"
    },
    "correct": "A"
  },
  {
    "id": 408,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 8,
    "text": "Xác định số vòng lặp cho bài toán: tính tổng các số nguyên từ 1 đến 100?",
    "options": {
      "A": "1",
      "B": "100",
      "C": "99",
      "D": "Tất cả đều sai"
    },
    "correct": "B"
  },
  {
    "id": 409,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 9,
    "text": "Đoạn chương trình sau giải bài toán nào?\n\nt = 0\nfor i in range(1, 101):\n    if (i % 3 == 0 and i % 5 == 0):\n        t = t + i\nprint(t)",
    "options": {
      "A": "Tính tổng các số chia hết cho 3 hoặc 5 trong phạm vi từ 1 đến 100",
      "B": "Tính tổng các số chia hết cho 3 hoặc 5 trong phạm vi từ 1 đến 101",
      "C": "Tính tổng các số chia hết cho 3 và 5 trong phạm vi từ 1 đến 101",
      "D": "Tính tổng các số chia hết cho 3 và 5 trong phạm vi từ 1 đến 100"
    },
    "correct": "D"
  },
  {
    "id": 410,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 10,
    "text": "Cho biết kết quả khi chạy đoạn chương trình sau:\n\ns = 0\nfor i in range(3):\n    s = s + 2*i\nprint(s)",
    "options": {
      "A": "12",
      "B": "10",
      "C": "8",
      "D": "6"
    },
    "correct": "D"
  },
  {
    "id": 411,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 11,
    "text": "Bạn An thực hiện đoạn chương trình sau nhưng chương trình báo lỗi. Theo em, bạn An sai ở dòng thứ mấy:\n\nnumbers = [6, 5, 3, 8, 4, 2, 5, 4, 11]\nsum == 0\nfor val in numbers:\n    sum = sum + val\nprint(\"The sum is\", sum)",
    "options": {
      "A": "4",
      "B": "3",
      "C": "2",
      "D": "5"
    },
    "correct": "C"
  },
  {
    "id": 412,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 12,
    "text": "Đoạn chương trình sau in ra kết quả nào?\n\nfor i in range(3):\n    print(i)",
    "options": {
      "A": "1 2 3",
      "B": "0 1 2",
      "C": "0 1 2 3",
      "D": "1 2"
    },
    "correct": "B"
  },
  {
    "id": 413,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 13,
    "text": "Lệnh range(2, 7) trong vòng lặp for biến đếm nhận những giá trị nào?",
    "options": {
      "A": "2 3 4 5 6 7",
      "B": "2 3 4 5 6",
      "C": "3 4 5 6 7",
      "D": "2 4 6"
    },
    "correct": "B"
  },
  {
    "id": 414,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 14,
    "text": "Cú pháp đúng của vòng lặp for trong Python là:",
    "options": {
      "A": "for i = 1 to 5: print(i)",
      "B": "for i in range(1,5) print(i)",
      "C": "for i in range(1,5): print(i)",
      "D": "for (i in range(1,5)) print(i)"
    },
    "correct": "C"
  },
  {
    "id": 415,
    "section": "Bài 4: Cấu trúc lặp for",
    "no": 15,
    "text": "Đoạn chương trình sau in ra bao nhiêu dòng?\n\nfor i in range(1, 6):\n    print(\"Hello\")",
    "options": {
      "A": "1",
      "B": "5",
      "C": "6",
      "D": "Không in ra gì"
    },
    "correct": "B"
  },
  {
    "id": 501,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 1,
    "text": "Trong các phát biểu sau, phát biểu nào chưa chính xác?",
    "options": {
      "A": "While là lệnh lặp với số lần không biết trước",
      "B": "For là lệnh lặp với số lần xác định trước",
      "C": "Khối lệnh lặp while được thực hiện cho đến khi <điều kiện> = False",
      "D": "While là lệnh lặp với số lần biết trước"
    },
    "correct": "D"
  },
  {
    "id": 502,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 2,
    "text": "Cho đoạn chương trình Python sau:\n\nTong = 0\nwhile Tong < 10:\n    Tong = Tong + 1\n\nSau khi đoạn chương trình trên được thực hiện, Tong nhận giá trị nào?",
    "options": {
      "A": "9",
      "B": "10",
      "C": "11",
      "D": "12"
    },
    "correct": "B"
  },
  {
    "id": 503,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 3,
    "text": "Cho biết kết quả của đoạn chương trình dưới đây:\n\na = 10\nwhile a < 11:\n    print(a)",
    "options": {
      "A": "Trên màn hình xuất hiện một số 10",
      "B": "Trên màn hình xuất hiện 10 chữ a",
      "C": "Trên màn hình xuất hiện một số 11",
      "D": "Chương trình bị lặp vô tận"
    },
    "correct": "D"
  },
  {
    "id": 504,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 4,
    "text": "Câu lệnh sau giải bài toán nào:\n\nwhile M != N:\n    if M > N:\n        M = M - N\n    else:\n        N = N - M",
    "options": {
      "A": "Tìm UCLN của M và N",
      "B": "Tìm BCNN của M và N.",
      "C": "Tìm hiệu nhỏ nhất của M và N",
      "D": "Tìm hiệu lớn nhất của M và N"
    },
    "correct": "A"
  },
  {
    "id": 505,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 5,
    "text": "Tính tổng S = 1 + 2 + 3 + 4 +… + n + … cho đến khi S>10000. Điều kiện nào sau đây cho vòng lặp while là đúng:",
    "options": {
      "A": "while S >= 10000:",
      "B": "while S < 10000:",
      "C": "while S <= 10000:",
      "D": "While S >10000"
    },
    "correct": "C"
  },
  {
    "id": 506,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 6,
    "text": "Hãy đưa ra kết quả trong đoạn lệnh sau:\n\nx = 1\nwhile (x <= 5):\n    print(\"python\")\n    x = x + 1",
    "options": {
      "A": "5 từ python",
      "B": "4 từ python",
      "C": "3 từ python",
      "D": "Không có kết quả"
    },
    "correct": "A"
  },
  {
    "id": 507,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 7,
    "text": "Cho đoạn chương trình sau:\n\nn = int(input(\"Nhập n<=1000: \"))\nk = 0\nn = abs(n)\nwhile n != 0:\n    n = n // 10\n    k = k + 1\nprint(k)\n\nChương trình có ý nghĩa gì?",
    "options": {
      "A": "k là số chữ số có nghĩa của n",
      "B": "k là chữ số hàng đơn vị của n",
      "C": "k là chữ số khác 0 lớn nhất của n",
      "D": "k là số chữ số khác 0 của n"
    },
    "correct": "A"
  },
  {
    "id": 508,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 8,
    "text": "Chọn đáp án đúng nhất:\n\ni = 0\nx = 0\nwhile i < 10:\n    if i % 2 == 0:\n        x += 1\n    i += 1\nprint(x)",
    "options": {
      "A": "2",
      "B": "3",
      "C": "4",
      "D": "5"
    },
    "correct": "D"
  },
  {
    "id": 509,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 9,
    "text": "Trong các phát biểu sau, phát biểu nào sai?",
    "options": {
      "A": "Có ba cấu trúc lập trình cơ bản của các ngôn ngữ lập trình",
      "B": "Cấu trúc tuần tự gồm các khối lệnh được thực hiện theo trình tự từ trên xuống dưới",
      "C": "Khối lệnh chỉ được thực hiện tuỳ thuộc điều kiện nào đó được thể hiện bằng câu lệnh for, while",
      "D": "Cả ba phương án trên đều đúng"
    },
    "correct": "C"
  },
  {
    "id": 510,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 10,
    "text": "Vòng lặp while – do kết thúc khi nào?",
    "options": {
      "A": "Khi một số điều kiện cho trước không còn thoả mãn",
      "B": "Khi đủ số vòng lặp",
      "C": "Khi tìm được output",
      "D": "Tất cả các phương án"
    },
    "correct": "A"
  },
  {
    "id": 511,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 11,
    "text": "Mọi quá trình tính toán đều có thể mô tả và thực hiện dựa trên cấu trúc cơ bản là:",
    "options": {
      "A": "Cấu trúc tuần tự",
      "B": "Cấu trúc rẽ nhánh",
      "C": "Cấu trúc lặp",
      "D": "Cả ba cấu trúc"
    },
    "correct": "D"
  },
  {
    "id": 512,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 12,
    "text": "Hoạt động nào sau đây lặp với số lần lặp chưa biết trước?",
    "options": {
      "A": "Ngày tắm hai lần",
      "B": "Học bài cho tới khi thuộc bài",
      "C": "Mỗi tuần đi nhà sách một lần",
      "D": "Ngày đánh răng hai lần"
    },
    "correct": "B"
  },
  {
    "id": 513,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 13,
    "text": "Cú pháp lệnh lặp với số lần chưa biết trước trong Python:",
    "options": {
      "A": "while <điều kiện> to <câu lệnh>",
      "B": "while <điều kiện> to <câu lệnh1> do<câu lệnh 2>",
      "C": "while <điều kiện> do: <câu lệnh>",
      "D": "while <điều kiện>: <câu lệnh>"
    },
    "correct": "D"
  },
  {
    "id": 514,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 14,
    "text": "Kết quả của chương trình sau:\n\nx = 1\ny = 5\nwhile x < y:\n    print(x, end=\" \")\n    x = x + 1",
    "options": {
      "A": "1 2 3 4",
      "B": "2 3 4 5",
      "C": "1 2 3 4 5",
      "D": "2 3 4"
    },
    "correct": "A"
  },
  {
    "id": 515,
    "section": "Bài 5: Câu lệnh lặp while",
    "no": 15,
    "text": "Kết quả của chương trình sau là gì?\n\nx = 8\ny = 2\nwhile y < x:\n    x = x - 2\n    print(x, end=\" \")",
    "options": {
      "A": "8, 6, 4, 2",
      "B": "8, 6, 4",
      "C": "6, 4, 2",
      "D": "8, 6, 4, 2, 0"
    },
    "correct": "C"
  }
];
  const TF_BANK = [
  {
    "id": 101,
    "topic": "Bài 1: Biến và lệnh gán (Đúng/Sai)",
    "no": 1,
    "text": "Cho đoạn thông tin: Trong Python, biến là một tên được gán cho một giá trị và có thể được sử dụng để lưu trữ dữ liệu. Lệnh gán sử dụng dấu \"=\" để gán giá trị cho biến. Ví dụ, x = 5 gán giá trị 5 cho biến x.",
    "items": [
      {
        "key": "a",
        "text": "Biến trong Python có thể thay đổi giá trị trong quá trình thực thi chương trình.",
        "correct": "D"
      },
      {
        "key": "b",
        "text": "Dấu \"=\" trong Python được sử dụng để so sánh hai giá trị.",
        "correct": "S"
      },
      {
        "key": "c",
        "text": "Một biến có thể lưu trữ nhiều loại dữ liệu khác nhau như số, chuỗi, danh sách.",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Biến không cần phải được khai báo trước khi sử dụng trong Python.",
        "correct": "D"
      }
    ]
  },
  {
    "id": 102,
    "topic": "Bài 1: Biến và lệnh gán (Đúng/Sai)",
    "no": 2,
    "text": "Cho đoạn thông tin: Python hỗ trợ nhiều phép toán cơ bản như cộng, trừ, nhân và chia. Các phép toán này được thực hiện bằng các ký hiệu như +, -, *, /. Python cũng hỗ trợ phép toán chia lấy nguyên với ký hiệu",
    "items": [
      {
        "key": "a",
        "text": "Ký hiệu * được sử dụng để thực hiện phép cộng trong Python.",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Phép toán chia lấy nguyên trong Python sử dụng ký hiệu //.",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Python không hỗ trợ phép toán trừ.",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Tất cả các phép toán cơ bản đều có thể được thực hiện với biến trong Python.",
        "correct": "D"
      }
    ]
  },
  {
    "id": 103,
    "topic": "Bài 1: Biến và lệnh gán (Đúng/Sai)",
    "no": 3,
    "text": "Cho đoạn thông tin: Trong Python, bạn có thể gán giá trị cho nhiều biến cùng lúc bằng cách sử dụng dấu phẩy. Ví dụ, a, b, c = 1, 2, 3 sẽ gán giá trị 1 cho a, 2 cho b và 3 cho c.",
    "items": [
      {
        "key": "a",
        "text": "Bạn không thể gán giá trị cho nhiều biến cùng lúc trong Python.",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Cách gán nhiều biến cùng lúc giúp mã nguồn trở nên ngắn gọn hơn.",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Tất cả các biến phải được gán giá trị khác nhau khi sử dụng cách gán này.",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Gán nhiều biến cùng lúc là một tính năng hữu ích trong Python.",
        "correct": "D"
      }
    ]
  },
  {
    "id": 104,
    "topic": "Bài 1: Biến và lệnh gán (Đúng/Sai)",
    "no": 4,
    "text": "Cho đoạn thông tin: Để làm việc với chuỗi trong Python, bạn có thể sử dụng các phép toán như nối chuỗi với dấu + và lặp chuỗi với dấu *. Ví dụ, \"Hello\" + \" World\" sẽ tạo ra chuỗi \"Hello World\".",
    "items": [
      {
        "key": "a",
        "text": "Dấu + được sử dụng để nối hai chuỗi lại với nhau.",
        "correct": "D"
      },
      {
        "key": "b",
        "text": "Dấu * không thể được sử dụng với chuỗi trong Python.",
        "correct": "S"
      },
      {
        "key": "c",
        "text": "Bạn có thể tạo ra chuỗi mới bằng cách lặp lại chuỗi cũ với dấu *.",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Chuỗi trong Python không thể được thay đổi sau khi đã được tạo ra.",
        "correct": "D"
      }
    ]
  },
  {
    "id": 105,
    "topic": "Bài 1: Biến và lệnh gán (Đúng/Sai)",
    "no": 5,
    "text": "Cho đoạn thông tin: Trong Python, có thể sử dụng lệnh gán để thay đổi giá trị của biến bất kỳ lúc nào. Điều này cho phép bạn cập nhật dữ liệu trong suốt quá trình thực thi chương trình.",
    "items": [
      {
        "key": "a",
        "text": "Giá trị của một biến không thể thay đổi sau khi đã được gán lần đầu tiên.",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Bạn có thể gán giá trị mới cho biến bất kỳ lúc nào trong chương trình.",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Lệnh gán là cách duy nhất để thay đổi giá trị của biến trong Python.",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Việc thay đổi giá trị của biến cho phép chương trình hoạt động linh hoạt hơn",
        "correct": "D"
      }
    ]
  },
  {
    "id": 201,
    "topic": "Bài 2: Các lệnh vào ra đơn giản (Đúng/Sai)",
    "no": 1,
    "text": "Cho đoạn thông tin: Trong Python, lệnh input() được sử dụng để nhận dữ liệu từ người dùng. Hàm này trả về một chuỗi (string) chứa dữ liệu mà người dùng đã nhập vào.",
    "items": [
      {
        "key": "a",
        "text": "Lệnh input() chỉ có thể nhận dữ liệu số",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Hàm input() luôn trả về dữ liệu dưới dạng chuỗi",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Người dùng không thể nhập dữ liệu vào chương trình nếu không có lệnh input()",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Lệnh input() có thể được sử dụng để nhận dữ liệu từ nhiều nguồn khác nhau",
        "correct": "S"
      }
    ]
  },
  {
    "id": 202,
    "topic": "Bài 2: Các lệnh vào ra đơn giản (Đúng/Sai)",
    "no": 2,
    "text": "Cho đoạn thông tin: Để xuất dữ liệu ra màn hình, Python cung cấp lệnh print(). Lệnh này có thể nhận nhiều đối số và in ra chúng trên cùng một dòng hoặc trên các dòng khác nhau.",
    "items": [
      {
        "key": "a",
        "text": "Lệnh print() chỉ có thể in ra một giá trị duy nhất",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Lệnh print() có thể in ra nhiều giá trị cùng lúc",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Dữ liệu được in ra bằng lệnh print() luôn nằm trên một dòng mới",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Có thể sử dụng lệnh print() để in ra các biến và chuỗi kết hợp với nhau",
        "correct": "D"
      }
    ]
  },
  {
    "id": 203,
    "topic": "Bài 2: Các lệnh vào ra đơn giản (Đúng/Sai)",
    "no": 3,
    "text": "Cho đoạn thông tin: Khi sử dụng lệnh input(), bạn có thể truyền vào một chuỗi để hiển thị như một thông báo cho người dùng biết cần nhập gì. Ví dụ: name = input(\"Nhập tên của bạn:\").",
    "items": [
      {
        "key": "a",
        "text": "Lệnh input() không thể hiển thị thông báo cho người dùng",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Bạn có thể tùy chỉnh thông báo của lệnh input()",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Dữ liệu nhập vào từ lệnh input() không thể được lưu vào biến",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Lệnh input() có thể được sử dụng để nhận nhiều loại dữ liệu khác nhau",
        "correct": "D"
      }
    ]
  },
  {
    "id": 204,
    "topic": "Bài 2: Các lệnh vào ra đơn giản (Đúng/Sai)",
    "no": 4,
    "text": "Cho đoạn thông tin: Để chuyển đổi dữ liệu nhập vào từ chuỗi sang kiểu số nguyên, bạn có thể sử dụng hàm int(). Ví dụ: age = int(input(\"Nhập tuổi của bạn: \")).",
    "items": [
      {
        "key": "a",
        "text": "Hàm int() có thể chuyển đổi chuỗi thành số nguyên",
        "correct": "D"
      },
      {
        "key": "b",
        "text": "Dữ liệu nhập vào từ lệnh input() luôn là kiểu số nguyên",
        "correct": "S"
      },
      {
        "key": "c",
        "text": "Nếu người dùng nhập vào một chuỗi không phải là số, hàm int() sẽ gây ra lỗi",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Bạn không thể chuyển đổi dữ liệu từ lệnh input() sang kiểu số thực",
        "correct": "S"
      }
    ]
  },
  {
    "id": 205,
    "topic": "Bài 2: Các lệnh vào ra đơn giản (Đúng/Sai)",
    "no": 5,
    "text": "Cho đoạn thông tin sau: Trong Python, để chương trình có thể giao tiếp với người dùng, ta thường sử dụng các lệnh vào ra cơ bản như input() và print(). Lệnh input() dùng để nhận dữ liệu từ bàn phím, còn lệnh print() dùng để hiển thị dữ liệu ra màn hình.",
    "items": [
      {
        "key": "a",
        "text": "Lệnh input() cho phép người dùng nhập dữ liệu từ bàn phím",
        "correct": "D"
      },
      {
        "key": "b",
        "text": "Dữ liệu nhận được từ lệnh input() luôn có kiểu số",
        "correct": "S"
      },
      {
        "key": "c",
        "text": "Lệnh print() có thể in ra nhiều giá trị trong cùng một lệnh",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Có thể sử dụng lệnh print() để in ra kết quả của một biểu thức",
        "correct": "D"
      }
    ]
  },
  {
    "id": 301,
    "topic": "Bài 3: Câu lệnh rẽ nhánh if (Đúng/Sai)",
    "no": 1,
    "text": "Cho đoạn thông tin: Câu lệnh rẽ nhánh if trong Python cho phép chương trình thực hiện các hành động khác nhau dựa trên điều kiện. Nếu điều kiện là đúng, mã lệnh trong khối if sẽ được thực hiện; nếu không, chương trình sẽ bỏ qua khối này.",
    "items": [
      {
        "key": "a",
        "text": "Câu lệnh if chỉ có thể kiểm tra một điều kiện duy nhất",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Nếu điều kiện trong câu lệnh if là sai, chương trình sẽ không thực hiện khối lệnh bên trong",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Câu lệnh if có thể được kết hợp với các câu lệnh else và elif",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Câu lệnh if không thể sử dụng với các loại dữ liệu khác nhau",
        "correct": "S"
      }
    ]
  },
  {
    "id": 302,
    "topic": "Bài 3: Câu lệnh rẽ nhánh if (Đúng/Sai)",
    "no": 2,
    "text": "Cho đoạn thông tin: Trong Python, cú pháp của câu lệnh if rất đơn giản. Một ví dụ cơ bản là: if x > 10: print(\"X lớn hơn 10\"). Điều này có nghĩa là nếu biến x lớn hơn 10, chương trình sẽ in ra thông báo.",
    "items": [
      {
        "key": "a",
        "text": "Cú pháp của câu lệnh if trong Python rất phức tạp và khó hiểu",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Câu lệnh if có thể sử dụng để kiểm tra nhiều điều kiện khác nhau",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Nếu x không lớn hơn 10, thông báo sẽ không được in ra",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Câu lệnh if không cần phải có dấu hai chấm (:) ở cuối",
        "correct": "S"
      }
    ]
  },
  {
    "id": 303,
    "topic": "Bài 3: Câu lệnh rẽ nhánh if (Đúng/Sai)",
    "no": 3,
    "text": "Cho đoạn thông tin: Câu lệnh if có thể được lồng nhau để kiểm tra nhiều điều kiện phức tạp. Điều này cho phép lập trình viên xây dựng các chương trình có logic phức tạp hơn.",
    "items": [
      {
        "key": "a",
        "text": "Câu lệnh if không thể lồng nhau trong Python",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Việc lồng câu lệnh if giúp kiểm tra nhiều điều kiện một cách hiệu quả",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Chỉ có thể lồng hai câu lệnh if với nhau",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Câu lệnh if giúp xây dựng logic phức tạp cho chương trình",
        "correct": "D"
      }
    ]
  },
  {
    "id": 304,
    "topic": "Bài 3: Câu lệnh rẽ nhánh if (Đúng/Sai)",
    "no": 4,
    "text": "Cho đoạn thông tin: Câu lệnh rẽ nhánh if là một phần quan trọng trong lập trình, cho phép chương trình quyết định các hành động dựa trên điều kiện cụ thể. Điều này rất hữu ích trong việc kiểm soát luồng thực thi của chương trình.",
    "items": [
      {
        "key": "a",
        "text": "Câu lệnh if không có tác động đến luồng thực thi của chương trình",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Câu lệnh if giúp kiểm soát hành động của chương trình dựa trên điều kiện",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Câu lệnh if cho phép thực hiện các phép toán trong khối lệnh khi điều kiện được thỏa mãn",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Việc không sử dụng câu lệnh if có thể làm cho chương trình trở nên kém linh hoạt",
        "correct": "D"
      }
    ]
  },
  {
    "id": 305,
    "topic": "Bài 3: Câu lệnh rẽ nhánh if (Đúng/Sai)",
    "no": 5,
    "text": "Cho đoạn thông tin: Trong Python, khi sử dụng câu lệnh if, có thể sử dụng các toán tử so sánh như ==, !=, >, <, >=, <= để kiểm tra giá trị của các biến",
    "items": [
      {
        "key": "a",
        "text": "Câu lệnh if chỉ có thể sử dụng toán tử so sánh ==",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Các toán tử so sánh khác nhau cho phép kiểm tra nhiều điều kiện khác nhau",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Câu lệnh if không thể so sánh các biến kiểu chuỗi",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Việc sử dụng toán tử so sánh trong câu lệnh if rất quan trọng để xác định điều kiện",
        "correct": "D"
      }
    ]
  },
  {
    "id": 401,
    "topic": "Bài 4: Cấu trúc lặp for (Đúng/Sai)",
    "no": 1,
    "text": "Cho đoạn thông tin: Câu lệnh lặp for trong Python dùng để lặp qua các phần tử của một đối tượng lặp (iterable) như danh sách, chuỗi, range,...",
    "items": [
      {
        "key": "a",
        "text": "Vòng lặp for chỉ có thể lặp qua danh sách (list)",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Vòng lặp for có thể lặp qua nhiều kiểu iterable (ví dụ: list, tuple, string, range)",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Có thể dùng for để lặp qua từng ký tự trong một chuỗi",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Với iterable hữu hạn (ví dụ range(10)), vòng lặp for sẽ luôn kết thúc",
        "correct": "D"
      }
    ]
  },
  {
    "id": 402,
    "topic": "Bài 4: Cấu trúc lặp for (Đúng/Sai)",
    "no": 2,
    "text": "Cho đoạn thông tin: Hàm range() thường được dùng trong for để tạo dãy số lặp.",
    "items": [
      {
        "key": "a",
        "text": "for i in range(5) sẽ lặp đúng 5 lần",
        "correct": "D"
      },
      {
        "key": "b",
        "text": "range(5) tạo ra dãy số từ 1 đến 5",
        "correct": "S"
      },
      {
        "key": "c",
        "text": "range(0, 10, 2) tạo ra dãy 0, 2, 4, 6, 8",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "range(2, 7) sẽ bao gồm cả số 7",
        "correct": "S"
      }
    ]
  },
  {
    "id": 403,
    "topic": "Bài 4: Cấu trúc lặp for (Đúng/Sai)",
    "no": 3,
    "text": "Cho đoạn thông tin: Có thể kết hợp for với if để lọc và xử lý dữ liệu",
    "items": [
      {
        "key": "a",
        "text": "Có thể dùng if bên trong vòng lặp for để kiểm tra điều kiện",
        "correct": "D"
      },
      {
        "key": "b",
        "text": "Câu lệnh for không thể kết hợp với câu lệnh if",
        "correct": "S"
      },
      {
        "key": "c",
        "text": "Có thể dùng vòng lặp for để tính tổng các phần tử trong một danh sách bằng cách cộng dồn",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Trong vòng lặp for, biến lặp luôn tăng đúng 1 sau mỗi lần lặp",
        "correct": "S"
      }
    ]
  },
  {
    "id": 404,
    "topic": "Bài 4: Cấu trúc lặp for (Đúng/Sai)",
    "no": 4,
    "text": "Cho đoạn chương trình sau: for i in range(1, 6): print(i)",
    "items": [
      {
        "key": "a",
        "text": "Vòng lặp for sẽ được thực hiện 5 lần",
        "correct": "D"
      },
      {
        "key": "b",
        "text": "Giá trị của i lần lượt nhận các giá trị từ 1 đến 5",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Chương trình sẽ in ra các số từ 0 đến 4",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Hàm range(1, 6) tạo ra dãy số từ 1 đến 6 (bao gồm cả 6)",
        "correct": "S"
      }
    ]
  },
  {
    "id": 405,
    "topic": "Bài 4: Cấu trúc lặp for (Đúng/Sai)",
    "no": 5,
    "text": "Cho đoạn thông tin: Sử dụng for, bạn có thể xử lý từng phần tử trong danh sách (ví dụ: tính tổng)",
    "items": [
      {
        "key": "a",
        "text": "Câu lệnh for không thể thực hiện phép toán trên từng phần tử trong danh sách",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Bạn có thể dùng một biến tích lũy (ví dụ: tong) để lưu tổng khi lặp qua danh sách",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Duyệt danh sách bằng for thì không thể thay đổi giá trị các phần tử của danh sách",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Câu lệnh for rất hữu ích để xử lý dữ liệu trong danh sách",
        "correct": "D"
      }
    ]
  },
  {
    "id": 501,
    "topic": "Bài 5: Câu lệnh lặp while (Đúng/Sai)",
    "no": 1,
    "text": "Cho đoạn thông tin: Câu lệnh lặp while trong Python cho phép thực hiện một khối lệnh nhiều lần miễn là điều kiện được cung cấp là đúng. Câu lệnh này thường được sử dụng khi số lần lặp không được xác định trước.",
    "items": [
      {
        "key": "a",
        "text": "Câu lệnh lặp while chỉ thực hiện một lần nếu điều kiện là đúng",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Câu lệnh lặp while có thể gây ra vòng lặp vô hạn nếu điều kiện không bao giờ trở thành sai",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Câu lệnh while có thể được sử dụng để lặp qua danh sách trong Python",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Điều kiện trong câu lệnh while là một biểu thức có giá trị logic (True hoặc False)",
        "correct": "D"
      }
    ]
  },
  {
    "id": 502,
    "topic": "Bài 5: Câu lệnh lặp while (Đúng/Sai)",
    "no": 2,
    "text": "Cho đoạn thông tin: Câu lệnh lặp while có cú pháp đơn giản: while điều_kiện: theo sau là khối lệnh cần thực hiện. Nếu điều kiện trở thành sai, vòng lặp sẽ kết thúc.",
    "items": [
      {
        "key": "a",
        "text": "Cú pháp của câu lệnh while yêu cầu phải có dấu hai chấm ở cuối dòng điều kiện",
        "correct": "D"
      },
      {
        "key": "b",
        "text": "Vòng lặp while không thể kết thúc nếu điều kiện luôn đúng",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Khối mã bên trong câu lệnh while có thể không cần phải thụt lề",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Câu lệnh while không thể được lồng vào nhau",
        "correct": "S"
      }
    ]
  },
  {
    "id": 503,
    "topic": "Bài 5: Câu lệnh lặp while (Đúng/Sai)",
    "no": 3,
    "text": "Câu lệnh lặp while cho phép thực hiện một khối lệnh nhiều lần khi điều kiện còn đúng",
    "items": [
      {
        "key": "a",
        "text": "Vòng lặp while chỉ kiểm tra điều kiện một lần trước khi thực hiện",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Nếu điều kiện ban đầu sai, khối lệnh trong while sẽ không được thực hiện lần nào",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Điều kiện trong câu lệnh while được kiểm tra trước mỗi lần lặp",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Câu lệnh while luôn thực hiện ít nhất một lần",
        "correct": "S"
      }
    ]
  },
  {
    "id": 504,
    "topic": "Bài 5: Câu lệnh lặp while (Đúng/Sai)",
    "no": 4,
    "text": "Xét đoạn chương trình sau: i = 1 while i <= 3: print(i) i += 1",
    "items": [
      {
        "key": "a",
        "text": "Vòng lặp while được thực hiện 3 lần",
        "correct": "D"
      },
      {
        "key": "b",
        "text": "Chương trình in ra các số 1, 2, 3",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Nếu bỏ dòng i += 1, chương trình sẽ bị lặp vô hạn",
        "correct": "D"
      },
      {
        "key": "d",
        "text": "Giá trị cuối cùng của biến i sau khi kết thúc vòng lặp là 3",
        "correct": "S"
      }
    ]
  },
  {
    "id": 505,
    "topic": "Bài 5: Câu lệnh lặp while (Đúng/Sai)",
    "no": 5,
    "text": "So sánh vòng lặp for và while trong Python:",
    "items": [
      {
        "key": "a",
        "text": "Vòng lặp while luôn thay thế được hoàn toàn vòng lặp for",
        "correct": "S"
      },
      {
        "key": "b",
        "text": "Vòng lặp while có thể sử dụng biến đếm để kiểm soát số lần lặp",
        "correct": "D"
      },
      {
        "key": "c",
        "text": "Vòng lặp while không thể duyệt qua các phần tử của danh sách",
        "correct": "S"
      },
      {
        "key": "d",
        "text": "Vòng lặp while linh hoạt hơn for trong việc kiểm soát điều kiện lặp",
        "correct": "D"
      }
    ]
  }
];

  // =========================
  // CẤU HÌNH ĐỀ KIỂM TRA
  // =========================
  const CONFIG = {
    // Số câu cho đề (đặt null để lấy TẤT CẢ)
    mcqCount: 15,  // ví dụ: 15 câu trắc nghiệm
    tfCount: 5,    // ví dụ: 5 câu đúng/sai (mỗi câu có 4 ý a,b,c,d)

    // Trộn đề ngẫu nhiên hay không
    shuffle: true,

    // Thời gian làm bài (phút). Đặt null để tự động = số câu (mcq + tf)
    durationMinutes: null,

    // Lưu đề + đáp án vào localStorage (refresh không mất)
    persistExam: true,
    persistAnswers: true,
  };

  // NOTE:
  // - Đây là file ôn luyện TỔNG HỢP (trộn Bài 1–5) nên cần storage key RIÊNG.
  // - Tránh bị "dính" đề/đáp án của Bài 1 (hoặc các bài khác) khiến HS thấy chỉ có khung / đề không đúng.
  const STORAGE_KEY = "exam_mix_state_v1";

  // =========================
  // STATE (localStorage)
  // =========================
  function loadState() {
    try {
      const raw = localStorage.getItem(STORAGE_KEY);
      if (!raw) return null;
      return JSON.parse(raw);
    } catch (e) {
      return null;
    }
  }

  function saveState(state) {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
    } catch (e) {}
  }

  function normalizeCount(count, max) {
    if (count === null || count === undefined) return max;
    const n = Number(count);
    if (!Number.isFinite(n) || n <= 0) return max;
    return Math.min(Math.floor(n), max);
  }

  function shuffleInPlace(arr) {
    for (let i = arr.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [arr[i], arr[j]] = [arr[j], arr[i]];
    }
    return arr;
  }

  function pickByIds(bank, ids) {
    const map = new Map(bank.map(q => [q.id, q]));
    return ids.map(id => map.get(id)).filter(Boolean);
  }

  function pickExamFromBank() {
    const desiredMcq = normalizeCount(CONFIG.mcqCount, MCQ_BANK.length);
    const desiredTf  = normalizeCount(CONFIG.tfCount, TF_BANK.length);

    const state = (CONFIG.persistExam || CONFIG.persistAnswers) ? (loadState() || {}) : {};

    // Nếu có đề đã lưu và hợp lệ thì dùng lại
    if (CONFIG.persistExam && state.exam && Array.isArray(state.exam.mcqIds) && Array.isArray(state.exam.tfIds)) {
      const mcqIds = state.exam.mcqIds;
      const tfIds  = state.exam.tfIds;

      const validLen = (mcqIds.length === desiredMcq) && (tfIds.length === desiredTf);
      const validIds =
        mcqIds.every(id => MCQ_BANK.some(q => q.id === id)) &&
        tfIds.every(id  => TF_BANK.some(q => q.id === id));

      if (validLen && validIds) {
        return {
          state,
          mcq: pickByIds(MCQ_BANK, mcqIds),
          tf:  pickByIds(TF_BANK, tfIds),
        };
      }
    }

    // =========================
    // Tạo đề mới (ÔN LUYỆN TỔNG HỢP)
    // - Trộn câu hỏi từ 5 bài
    // - Vẫn giữ tổng: 15 trắc nghiệm + 5 đúng/sai
    // - Đảm bảo "đủ 5 dạng": mỗi bài góp 1 phần (mặc định 3 TN + 1 Đ/S)
    // =========================
    const topicDefs = [
      { key: "Bài 1", mcq: MCQ_BANK.filter(q => String(q.section||"").startsWith("Bài 1:")), tf: TF_BANK.filter(q => String(q.topic||"").startsWith("Bài 1:")) },
      { key: "Bài 2", mcq: MCQ_BANK.filter(q => String(q.section||"").startsWith("Bài 2:")), tf: TF_BANK.filter(q => String(q.topic||"").startsWith("Bài 2:")) },
      { key: "Bài 3", mcq: MCQ_BANK.filter(q => String(q.section||"").startsWith("Bài 3:")), tf: TF_BANK.filter(q => String(q.topic||"").startsWith("Bài 3:")) },
      { key: "Bài 4", mcq: MCQ_BANK.filter(q => String(q.section||"").startsWith("Bài 4:")), tf: TF_BANK.filter(q => String(q.topic||"").startsWith("Bài 4:")) },
      { key: "Bài 5", mcq: MCQ_BANK.filter(q => String(q.section||"").startsWith("Bài 5:")), tf: TF_BANK.filter(q => String(q.topic||"").startsWith("Bài 5:")) },
    ];

    const perMcq = Math.max(1, Math.floor(desiredMcq / topicDefs.length));
    const perTf  = Math.max(1, Math.floor(desiredTf  / topicDefs.length));

    const mcq = [];
    const tf  = [];

    for (const t of topicDefs) {
      const mPool = (t.mcq || []).slice();
      const tPool = (t.tf  || []).slice();

      if (CONFIG.shuffle) {
        shuffleInPlace(mPool);
        shuffleInPlace(tPool);
      }

      mcq.push(...mPool.slice(0, perMcq));
      tf.push(...tPool.slice(0, perTf));
    }

    // Bù nếu thiếu (trong trường hợp bank ít hơn quota)
    if (mcq.length < desiredMcq) {
      const rest = MCQ_BANK.filter(q => !mcq.some(x => x.id === q.id));
      if (CONFIG.shuffle) shuffleInPlace(rest);
      mcq.push(...rest.slice(0, desiredMcq - mcq.length));
    }
    if (tf.length < desiredTf) {
      const rest = TF_BANK.filter(q => !tf.some(x => x.id === q.id));
      if (CONFIG.shuffle) shuffleInPlace(rest);
      tf.push(...rest.slice(0, desiredTf - tf.length));
    }

    // Cắt cho đúng số câu
    if (mcq.length > desiredMcq) mcq.length = desiredMcq;
    if (tf.length  > desiredTf)  tf.length  = desiredTf;

    // Trộn thứ tự hiển thị
    if (CONFIG.shuffle) {
      shuffleInPlace(mcq);
      shuffleInPlace(tf);
    }

    if (CONFIG.persistExam || CONFIG.persistAnswers) {
      state.exam = {
        mcqIds: mcq.map(q => q.id),
        tfIds: tf.map(q => q.id),
      };
      state.answers = state.answers || {};
      state.submitted = false;
      saveState(state);
    }

    return { state, mcq, tf };
  }

  // =========================
  // RENDER QUESTIONS
  // =========================
  const part1El = document.getElementById("part1-questions");
  const part2El = document.getElementById("part2-questions");
  const part1Title = document.getElementById("part1-title");
  const part2Title = document.getElementById("part2-title");
  const examMeta = document.getElementById("examMeta");

  function el(tag, className, text) {
    const e = document.createElement(tag);
    if (className) e.className = className;
    if (text !== undefined) e.textContent = text;
    return e;
  }

  function renderMCQ(mcqList) {
    part1El.innerHTML = "";

    // Group theo section để nhìn dễ
    const groups = new Map();
    for (const q of mcqList) {
      const key = q.section || "Trắc nghiệm";
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key).push(q);
    }

    let idx = 0;
    for (const [section, qs] of groups.entries()) {
      const sub = el("div", "sub-header", section);
      part1El.appendChild(sub);

      for (const q of qs) {
        idx++;
        const card = el("div", "question-card");
        card.id = `mcq-${q.id}`;
        card.dataset.sideLabel = `TN ${idx}`;
        card.dataset.sideDesc = (q.text || "").slice(0, 60);

        const meta = el("div", "q-meta");
        meta.appendChild(el("span", "tag", `Mã: MCQ-${q.id}`));
        card.appendChild(meta);

        const qt = el("span", "question-text");
        const num = el("span", "question-num", `Câu ${idx}:`);
        qt.appendChild(num);
        qt.appendChild(document.createTextNode(" " + (q.text || "")));
        card.appendChild(qt);

        const optList = el("div", "options-list");
        const name = `mcq_${q.id}`;

        for (const key of ["A","B","C","D"]) {
          const label = el("label", "option-item");
          const input = document.createElement("input");
          input.type = "radio";
          input.name = name;
          input.value = key;

          const span = document.createTextNode(` ${key}. ${q.options[key] || ""}`);
          label.appendChild(input);
          label.appendChild(span);

          optList.appendChild(label);
        }

        card.appendChild(optList);
        part1El.appendChild(card);
      }
    }

    part1Title.textContent = `Phần I: Trắc nghiệm 4 lựa chọn (${mcqList.length} câu)`;
  }

  function renderTF(tfList) {
    part2El.innerHTML = "";

    const groups = new Map();
    for (const q of tfList) {
      const key = q.topic || "Đúng/Sai";
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key).push(q);
    }

    let idx = 0;
    for (const [topic, qs] of groups.entries()) {
      const sub = el("div", "sub-header", topic);
      part2El.appendChild(sub);

      for (const q of qs) {
        idx++;
        const card = el("div", "question-card");
        card.id = `tf-${q.id}`;
        card.dataset.sideLabel = `ĐS ${idx}`;
        card.dataset.sideDesc = (q.text || "").slice(0, 60);

        const meta = el("div", "q-meta");
        meta.appendChild(el("span", "tag", `Mã: TF-${q.id}`));
        card.appendChild(meta);

        const qt = el("span", "question-text");
        const num = el("span", "question-num", `Câu ${idx}:`);
        qt.appendChild(num);
        qt.appendChild(document.createTextNode(" " + (q.text || "")));
        card.appendChild(qt);

        // Items a,b,c,d
        const items = Array.isArray(q.items) ? q.items : [];
        for (const it of items) {
          const row = el("div", "tf-row");

          const label = el("div", "tf-label", `${it.key}) ${it.text || ""}`);
          row.appendChild(label);

          const opt = el("div", "tf-options");
          const name = `tf_${q.id}_${it.key}`;

          // Đúng
          {
            const wrap = document.createElement("label");
            const input = document.createElement("input");
            input.type = "radio";
            input.name = name;
            input.value = "D";
            const btn = el("span", "tf-btn", "Đúng");
            wrap.appendChild(input);
            wrap.appendChild(btn);
            opt.appendChild(wrap);
          }
          // Sai
          {
            const wrap = document.createElement("label");
            const input = document.createElement("input");
            input.type = "radio";
            input.name = name;
            input.value = "S";
            const btn = el("span", "tf-btn", "Sai");
            wrap.appendChild(input);
            wrap.appendChild(btn);
            opt.appendChild(wrap);
          }

          row.appendChild(opt);
          card.appendChild(row);
        }

        part2El.appendChild(card);
      }
    }

    part2Title.textContent = `Phần II: Câu hỏi Đúng/Sai (${tfList.length} câu)`;
  }

  function applySavedAnswers(state) {
    if (!CONFIG.persistAnswers || !state || !state.answers) return;
    const answers = state.answers;

    for (const name in answers) {
      const value = answers[name];
      const inputs = document.querySelectorAll(`input[type="radio"][name="${name}"]`);
      inputs.forEach(inp => {
        if (inp.value === value) inp.checked = true;
      });
    }
  }

  // =========================
  // SIDEBAR + PROGRESS
  // =========================
  const sideList = document.getElementById('sideList');
  const sidePill = document.getElementById('side-pill');
  const barFill = document.getElementById('barFill');
  const doneTxt = document.getElementById('doneTxt');
  const totalTxt = document.getElementById('totalTxt');

  function getQuestionLabel(card, index) {
    const title = card.dataset.sideLabel || `Câu ${index + 1}`;
    const desc  = card.dataset.sideDesc || "";
    return { title, desc };
  }

  function escAttr(s) {
    return String(s == null ? "" : s)
      .replace(/&/g, "&amp;")
      .replace(/"/g, "&quot;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");
  }


  function getCardStatus(card) {
    const radios = Array.from(card.querySelectorAll('input[type="radio"]'));
    if (radios.length === 0) return 'todo';

    const names = Array.from(new Set(radios.map(r => r.name).filter(Boolean)));
    if (names.length === 0) return 'todo';

    let answered = 0;
    for (const name of names) {
      const checked = card.querySelector(`input[type="radio"][name="${name}"]:checked`);
      if (checked) answered++;
    }

    if (answered === 0) return 'todo';
    if (answered < names.length) return 'partial';
    return 'done';
  }

  function buildSidebar() {
    const cards = Array.from(document.querySelectorAll('.question-card'));

    sideList.innerHTML = cards.map((card, idx) => {
      if (!card.id) card.id = `qcard-${idx + 1}`;
      const { title, desc } = getQuestionLabel(card, idx);
      const status = getCardStatus(card);
      const badgeText = status === 'done' ? 'Đã làm' : (status === 'partial' ? 'Đang làm' : 'Chưa làm');

      return `
        <div class="side-item" data-target="${card.id}" data-status="${status}" title="${escAttr(desc)}">
          <div class="side-left">
            <div class="side-idx">${idx + 1}</div>
            <div class="side-label">${title}</div>
          </div>
          <div class="badge">${badgeText}</div>
        </div>
      `;
    }).join('');

    updateProgress();
    setupActiveObserver();
  }

  function updateProgress() {
    const items = Array.from(sideList.querySelectorAll('.side-item'));
    const cards = items.map(it => document.getElementById(it.dataset.target)).filter(Boolean);

    let doneCount = 0;
    cards.forEach((card, i) => {
      const status = getCardStatus(card);
      const item = items[i];
      if (!item) return;

      item.dataset.status = status;
      const badge = item.querySelector('.badge');
      if (badge) badge.textContent = status === 'done' ? 'Đã làm' : (status === 'partial' ? 'Đang làm' : 'Chưa làm');
      if (status === 'done') doneCount++;
    });

    const total = cards.length || 0;
    const pct = total ? Math.round((doneCount / total) * 100) : 0;

    sidePill.textContent = `${doneCount}/${total}`;
    doneTxt.textContent = `${doneCount} đã làm`;
    totalTxt.textContent = `${total} tổng`;
    barFill.style.width = `${pct}%`;
  }

  // Highlight item đang nhìn
  let observer = null;
  function setupActiveObserver() {
    if (observer) observer.disconnect();
    const items = Array.from(sideList.querySelectorAll('.side-item'));
    const map = new Map(items.map(it => [it.dataset.target, it]));
    const cards = Array.from(document.querySelectorAll('.question-card'));

    observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (!entry.isIntersecting) return;
        const id = entry.target.id;
        items.forEach(it => it.classList.remove('active'));
        const it = map.get(id);
        if (it) it.classList.add('active');
      });
    }, {
      root: null,
      threshold: 0.25
    });

    cards.forEach(c => observer.observe(c));
  }

  // Nhảy tới câu khi click sidebar
  document.addEventListener('click', (e) => {
    const item = e.target.closest('.side-item[data-target]');
    if (!item) return;
    const id = item.dataset.target;
    const el = document.getElementById(id);
    if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' });
  });

  // =========================
  // TIMER
  // =========================
  let timeLeft = 0;
  const timerDisplay = document.getElementById('timer');
  let countdown = null;

  function formatTime(sec) {
    const m = Math.floor(sec / 60);
    const s = sec % 60;
    return `${m}:${s < 10 ? '0' : ''}${s}`;
  }

  function startTimer(totalMinutes) {
    timeLeft = Math.max(0, Math.floor(totalMinutes * 60));
    timerDisplay.textContent = formatTime(timeLeft);

    if (countdown) clearInterval(countdown);
    countdown = setInterval(() => {
      timerDisplay.textContent = formatTime(timeLeft);

      if (timeLeft <= 0) {
        clearInterval(countdown);
        alert("Hết giờ! Hệ thống sẽ tự động nộp bài.");
        submitTest();
        return;
      }
      timeLeft--;
    }, 1000);
  }

  // =========================
  // SUBMIT & SCORING
  // =========================
  let EXAM = null; // { state, mcq, tf }

  function submitTest(fromRestore = false) {
    if (!EXAM) return;
    if (countdown) clearInterval(countdown);

    const mcqList = EXAM.mcq;
    const tfList  = EXAM.tf;

    let mcqCorrect = 0;
    let mcqAnswered = 0;

    for (const q of mcqList) {
      const name = `mcq_${q.id}`;
      const checked = document.querySelector(`input[name="${name}"]:checked`);
      if (checked) {
        mcqAnswered++;
        if (checked.value === q.correct) mcqCorrect++;
      }
    }

    let tfItemsTotal = 0;
    let tfItemsCorrect = 0;
    let tfItemsAnswered = 0;

    // Mỗi câu Đ/S quy về 1 điểm (chia đều theo số ý của câu đó)
    let tfPointsTotal = 0;
    let tfPointsEarned = 0;

    for (const q of tfList) {
      const items = Array.isArray(q.items) ? q.items : [];
      if (!items.length) continue;

      tfPointsTotal += 1;
      const perItem = 1 / items.length;

      for (const it of items) {
        tfItemsTotal++;
        const name = `tf_${q.id}_${it.key}`;
        const checked = document.querySelector(`input[name="${name}"]:checked`);
        if (checked) {
          tfItemsAnswered++;
          if (checked.value === it.correct) {
            tfItemsCorrect++;
            tfPointsEarned += perItem;
          }
        }
      }
    }

    // Quy đổi điểm:
    // - Mỗi câu trắc nghiệm đúng = 1 điểm
    // - Mỗi câu đúng/sai đúng tối đa = 1 điểm (tính theo số ý đúng)
    const totalPoints = mcqList.length + tfPointsTotal;
    const earnedPoints = mcqCorrect + tfPointsEarned;
    const score10 = totalPoints ? (earnedPoints / totalPoints) * 10 : 0;

    const passed = score10 >= PRETEST_PASS_SCORE;

    // Lưu kết quả (kể cả chưa đạt) để GV xem trong trang Kết quả
    try{
      savePretestResult({
        passed: !!passed,
        score: Number(score10||0),
        detail: {
          mcqCorrect: mcqCorrect,
          mcqTotal: mcqList.length,
          mcqAnswered: mcqAnswered,
          tfItemsCorrect: tfItemsCorrect,
          tfItemsTotal: tfItemsTotal,
          tfItemsAnswered: tfItemsAnswered
        }
      }, { countAttempt: !fromRestore });
    }catch(e){}

    // Hiển thị kết quả
    document.getElementById('btn-submit').style.display = 'none';
    const resultBox = document.getElementById('result-box');
    resultBox.style.display = 'block';

    document.getElementById('score-text').innerText = score10.toFixed(2) + " Điểm";
    document.getElementById('details-text').innerText =
      `Trắc nghiệm: đúng ${mcqCorrect}/${mcqList.length} (đã làm ${mcqAnswered}/${mcqList.length}) • ` +
      `Đúng/Sai: đúng ${tfItemsCorrect}/${tfItemsTotal} ý (đã làm ${tfItemsAnswered}/${tfItemsTotal} ý)`;

    // Khóa chọn sau khi nộp
    document.querySelectorAll('input[type="radio"]').forEach(i => i.disabled = true);

    // Hiển thị đáp án đúng/sai trực quan
    revealAnswers();

    // Lưu trạng thái submitted
    if (CONFIG.persistExam || CONFIG.persistAnswers) {
      EXAM.state.submitted = true;
      saveState(EXAM.state);
    }

        // =========================
    // PY10 PRETEST NOTE: Đạt yêu cầu => mở khóa phần CODE ở trang chính
    // =========================
    try{
      const old = document.getElementById("pretestNote");
      if(old) old.remove();
      const note = document.createElement("div");
      note.id = "pretestNote";
      note.style.marginTop = "10px";
      note.style.fontSize = "12px";
      note.style.fontWeight = "900";
      if(score10 >= PRETEST_PASS_SCORE){
        note.style.color = "#0f172a";
        note.innerHTML = `✅ Bạn đã đạt yêu cầu (≥ ${PRETEST_PASS_SCORE.toFixed(1)} điểm). Bấm <b>“LÀM CODE”</b> bên dưới để bắt đầu.`;
        markPretestPassed(score10, { preserveTs: fromRestore });
      }else{
        note.style.color = "#b00020";
        note.innerHTML = `❌ Chưa đạt yêu cầu (cần ≥ ${PRETEST_PASS_SCORE.toFixed(1)} điểm). Hãy bấm “Tạo đề mới” để làm lại.`;
      }
      resultBox.appendChild(note);
    }catch(e){}

    // Nếu đã đạt yêu cầu, hiển thị nút "Làm code" để HS chủ động quay lại phần code
    try{
      const oldBtn = document.getElementById("btnGoCode");
      if(oldBtn) oldBtn.remove();
      if(passed){
        const btn = document.createElement("button");
        btn.id = "btnGoCode";
        btn.className = "btn-submit";
        btn.style.marginTop = "12px";
        btn.style.background = "linear-gradient(135deg, #16a34a, #22c55e)";
        btn.textContent = "LÀM CODE";
        btn.onclick = goToCode;
        resultBox.appendChild(btn);
      }
    }catch(e){}

updateProgress();
    if (!fromRestore) window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
  }

  
  // =========================
  // HIỂN THỊ ĐÁP ÁN (sau khi nộp)
  // =========================
  function clearReveal() {
    document.querySelectorAll('.option-item.correct, .option-item.wrong').forEach(el => {
      el.classList.remove('correct', 'wrong');
    });
    document.querySelectorAll('.tf-btn.correct, .tf-btn.wrong').forEach(el => {
      el.classList.remove('correct', 'wrong');
    });
    document.querySelectorAll('.answer-note').forEach(el => el.remove());
  }

  function revealAnswers() {
    if (!EXAM) return;

    // MCQ: tô màu đáp án đúng + đáp án chọn sai (nếu có)
    for (const q of EXAM.mcq) {
      const card = document.getElementById(`mcq-${q.id}`);
      if (!card) continue;

      const correct = q.correct;

      // mark correct option
      card.querySelectorAll('input[type="radio"]').forEach(inp => {
        const label = inp.closest('label');
        if (!label) return;
        if (inp.value === correct) label.classList.add('correct');
      });

      // mark wrong chosen
      const chosen = card.querySelector(`input[name="mcq_${q.id}"]:checked`);
      if (chosen && chosen.value !== correct) {
        const label = chosen.closest('label');
        if (label) label.classList.add('wrong');
      }

      // show note
      let note = card.querySelector('.answer-note');
      if (!note) {
        note = document.createElement('div');
        note.className = 'answer-note';
        card.appendChild(note);
      }
      note.innerHTML = `Đáp án đúng: <b>${correct}</b>`;
    }

    // TF: tô màu nút Đ/S đúng và tô màu lựa chọn sai (nếu có)
    for (const q of EXAM.tf) {
      const card = document.getElementById(`tf-${q.id}`);
      if (!card) continue;

      const items = Array.isArray(q.items) ? q.items : [];
      for (const it of items) {
        const name = `tf_${q.id}_${it.key}`;
        const correctVal = it.correct;

        const correctInput = card.querySelector(`input[name="${name}"][value="${correctVal}"]`);
        if (correctInput && correctInput.nextElementSibling) {
          correctInput.nextElementSibling.classList.add('correct');
        }

        const chosen = card.querySelector(`input[name="${name}"]:checked`);
        if (chosen && chosen.value !== correctVal) {
          if (chosen.nextElementSibling) chosen.nextElementSibling.classList.add('wrong');
        }
      }
    }
  }

// =========================
  // EVENTS: lưu đáp án + nút tiện ích
  // =========================
  document.addEventListener('change', (e) => {
    if (e.target && e.target.matches('input[type="radio"]')) {
      if (CONFIG.persistAnswers && EXAM && EXAM.state) {
        EXAM.state.answers = EXAM.state.answers || {};
        EXAM.state.answers[e.target.name] = e.target.value;
        saveState(EXAM.state);
      }
      updateProgress();
    }
  });

  document.getElementById("btn-new").addEventListener("click", () => {
    // tạo đề mới: xoá localStorage và reload
    localStorage.removeItem(STORAGE_KEY);
    location.reload();
  });

  document.getElementById("btn-clear").addEventListener("click", () => {
    // xoá đáp án nhưng giữ đề
    if (CONFIG.persistAnswers && EXAM && EXAM.state) {
      EXAM.state.answers = {};
      EXAM.state.submitted = false;
      saveState(EXAM.state);
    }
    document.querySelectorAll('input[type="radio"]').forEach(i => {
      i.checked = false;
      i.disabled = false;
    });
    clearReveal();
    document.getElementById('btn-submit').style.display = 'block';
    document.getElementById('result-box').style.display = 'none';
    if (countdown) clearInterval(countdown);

    // restart timer
    const autoMinutes = (CONFIG.durationMinutes === null || CONFIG.durationMinutes === undefined)
      ? (EXAM.mcq.length + EXAM.tf.length)
      : Number(CONFIG.durationMinutes);

    startTimer(autoMinutes);
    updateProgress();
    window.scrollTo({ top: 0, behavior: 'smooth' });
  });

  // =========================
  // INIT
  // =========================
  function init() {
    EXAM = pickExamFromBank();

    // Render
    renderMCQ(EXAM.mcq);
    renderTF(EXAM.tf);

    // Meta header
    const totalCards = EXAM.mcq.length + EXAM.tf.length;
    // Header meta: giúp HS biết đây là đề TỔNG HỢP (không phải Bài 1)
    examMeta.textContent = `Đề: ${EXAM.mcq.length} TN + ${EXAM.tf.length} Đ/S (tổng ${totalCards} câu)`;

    // Apply answers
    applySavedAnswers(EXAM.state);

    // Sidebar
    buildSidebar();
    updateProgress();

    // Timer (auto theo số câu nếu durationMinutes = null)
    const minutes = (CONFIG.durationMinutes === null || CONFIG.durationMinutes === undefined)
      ? (EXAM.mcq.length + EXAM.tf.length)
      : Number(CONFIG.durationMinutes);

    // Nếu đã submit trước đó => tự hiển thị lại kết quả (không chạy timer)
    if (EXAM.state && EXAM.state.submitted) {
      timerDisplay.textContent = "Đã nộp";
      submitTest(true);
      return;
    }

    startTimer(minutes);
  }

  init();
